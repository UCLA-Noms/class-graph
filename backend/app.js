...
app.get("/getcoursesbycat/:major/:category", async (req, res) => {
    ...
    res.send(
        { // TODO: Figure out how to map course to multiple professors
            "course_nodes": course_result.records.filter(record => record._fields[0].labels[0] === "Course").map(record => ({"class_code": record._fields[0].properties.CourseID, "class_name": record._fields[0].properties.Name, "class_description": record._fields[0].properties.Description, "class_units": record._fields[0].properties.Units, "professors": course_result.records.filter(r => (r._fields[0].labels[0] === "Course" && r._fields[0].properties.CourseID === record._fields[0].properties.CourseID)).map(record => record._fields[1].properties.Professor), "id": record._fields[0].identity.low})),
            ...
            "prereq_edges": prereq_result.records.map(record => ({"prereq": record._fields[0].identity.low,"postreq": record._fields[1].identity.low}))
        }    
            );
})
